...
<ParamField body="unlockTime" type="integer" required={false} default="8">
  How long the door stays unlocked, in seconds. If supplied, it must be between
  2 and 60.
</ParamField>

| Condition                                           | Behavior                                                   |
| --------------------------------------------------- | ---------------------------------------------------------- |
| Supplied value from 2 through 60                    | Uses the supplied duration.                                |
| Supplied value outside 2 through 60                 | Returns `400`.                                             |
| Omitted with an access-point duration configured    | Uses the configured duration, constrained to 2–60 seconds. |
| Omitted without an access-point duration configured | Uses 8 seconds.                                            |
...
| 422    | Location missing Roblox Universe ID or effective Open Cloud API key | Configure the location's Universe ID and either an organization default key or location override in the XCS dashboard.                                              |
